...
const putObjectMultipart = async (
  client: S3Client,
  bucketName: string,
  filePath: string,
  file: File
...
function App() {
  const [current, setCurrent] = useState<RestNode>({
    Uuid: "",
    Path: "/",
    Type: "COLLECTION",
  });
  ...
  const [showSettings, setShowSettings] = useState<boolean>(
    localSettings ? localSettings === "true" : true
  );
  const [basePath, setBasePath] = useState<string>(
    localStorage.getItem("basePath") || ""
  );
  const [restSegment, setRestSegment] = useState<string>(
    localStorage.getItem("restSegment") || "/a"
  );
  const [s3URL, setS3URL] = useState<string>(
    localStorage.getItem("s3URL") || ""
  );
  const [s3Bucket, setS3Bucket] = useState<string>(
    localStorage.getItem("s3Bucket") || "io"
  );
  ...
  const createNode = (type: string) => {
    const name = window.prompt(
      "Name?",
      type === "folder" ? "New Folder" : "Empty File.txt"
    );
    if (!name) {
      return;
    }
  ...
  useEffect(() => {
    setSelection("");
    loadCurrent();
  }, [current, apiKey, basePath, restSegment]);
  useEffect(() => {
  ...
  return (
    <>
      <div>
        ...
        >
          Api Settings&nbsp;<a>{showSettings ? "-" : "+"}</a>
        </h4>
        <div style={{ display: showSettings ? "block" : "none" }}>
          <div style={{ height: 26, display: "flex", alignItems: "center" }}>
            <label
              style={{ width: 100, fontSize: 12, margin: "0 10px" }}
              htmlFor={"input-rest"}
            >
              REST Endpoint
            </label>
            <input
              ...
              style={{ width: 300 }}
              type={"text"}
              placeholder={"Full URL without trailing slash"}
              value={basePath}
            ...
            <input
              style={{ width: 100 }}
              type={"text"}
              placeholder={"REST API main endpoint"}
              value={restSegment}
      ...
      <div style={{ display: "flex", alignItems: "center" }}>
        <h2 style={{ flex: 1 }}>
          {insideWorkspace ? "Folder " + current.Path : "Workspaces"}{" "}
          {loading && "⏳"}
        </h2>
        ...
        <>
          ...
          >
            ...
            <div style={{ display: "flex" }}>
              <div style={{ flex: 1 }}>
                Drag 'n' drop some files here, or <a>click to select files</a>
              </div>
              <input
                type={"checkbox"}
                id={"multipart"}
                checked={useMultipart}
                onChange={() => setUseMultipart(!useMultipart)}
                onClick={(e) => {
                  e.stopPropagation();
                }}
              />
              ...
              <input
                ...
                onChange={() => setRenameExisting(!renameExisting)}
                onClick={(e) => {
                  e.stopPropagation();
                }}
              />
      ...
      <div className="card" style={{ display: "flex", alignItems: "start" }}>
        <div style={{ flex: 1 }}>
          <div style={{ overflowX: "auto" }}>
            {insideWorkspace && (
              <div
                onClick={() => setCurrent(getParent(current))}
                style={{ cursor: "pointer" }}
              >
                ⬆️ ..
              </div>
      ...
      <p className="read-the-docs">
        Wicked ! Made by Charles with love - [Hint] Right-click on folders to
        show their info
      </p>
    </>
  );
